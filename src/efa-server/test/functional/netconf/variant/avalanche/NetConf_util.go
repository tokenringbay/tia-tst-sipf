...
func (v *Variant) TConfigureMacAndArp(t *testing.T, Host string, Model string) {
	...
	assert.Equal(t, map[string]string{"mac-aging-timeout": macAgingTimeout, "mac-conversational-timeout": "300"}, responses)
	assert.Equal(t, "<ok/>", msg, "")
	assert.Nil(t, err, "")
...
func (v *Variant) TMCTClusterConfigureMCTCluster(t *testing.T, Host string, Model string) {
	...
	assert.Equal(t, map[string]string{"cluster-id": clusterID, "peer-type": "Ve", "peer-name": controlVe, "peer-ip": peerLoopbackIP,
		"deploy": "true", "client-isolation-loose": "true"}, ResultMap, "")
