...
//UnconfigureBGP is used to unconfigure "router bgp" config on the switch.
func UnconfigureBGP(ctx context.Context, wg *sync.WaitGroup, sw *operation.ConfigSwitch, errs chan actions.OperationError) {
	defer wg.Done()
	...
	if len(bgpNeighborResponseMap) == 0 {
		Operation := "BGP Route Map Operation"
		x, err := adapter.UnConfigureNumberedRouteMap(client)
		if err == nil {
			log.Infof("BGP Route Map Unconfiguration Completed: %s\n", x)
		} else {
			log.Errorf("BGP Route MapUnconfiguration Failed: %s\n", err)
			errs <- actions.OperationError{Operation: Operation, Error: errors.New(Operation + ":" + err.Error()), Host: sw.Host}
			return
		}
